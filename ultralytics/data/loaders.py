...
class LoadTensorBuffer:
    ...
    def __init__(self, im0, buffer) -> None:
        ...
        self.im0 = self._single_check(im0)
        self.buffer = buffer
        # if buffer != [None,None,None]:
        #     if buffer[0].shape[1] < buffer[0].shape[2]: #must torch.Size([1, 224, 224, 64]), error torch.Size([1, 64, 224, 224])
        #         buffer = [b.permute(0, 2, 3, 1) for b in buffer]
        self.bs = self.im0.shape[0]
        self.mode = "image"
        self.paths = [getattr(im, "filename", f"image{i}.jpg") for i, im in enumerate(im0)]
