...
class StreamSampler(Sampler):
    ...
    def __iter__(self):
        ...
        indices = list(range(len(self.dataset.sub_videos_mapping)))
        ...
        # Apply distributed sampling
        if self.distributed:
            indices = indices[self.rank:self.world_size:self.world_size]
        ...
        assert len(indices) == (len(indices) // self.batch_size) * self.batch_size, "Number of sub-videos is not divisible by batch size"
