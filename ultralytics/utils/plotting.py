...
def plot_video(
    ...
    # 创建 VideoWriter 对象
    # video_writer = cv2.VideoWriter(str(fname), cv2.VideoWriter_fourcc(*"mp4v"), 10, (1920, 1920))
    init_flag = False
    for k, images in enumerate(video_batch_list):
        ...
        # if on_plot:
        #     on_plot(fname)
        img_array = cv2.cvtColor(np.array(annotator.im), cv2.COLOR_RGB2BGR)
        # cv2.imwrite(os.path.join(save_dir,f"{k}.png"), img_array)
        if not init_flag:
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    """
    for m in {"Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "DyDetect"}:  # all model heads
        if m in module_type:
            return
