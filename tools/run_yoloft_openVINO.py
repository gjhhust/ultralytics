...
def load_openvino_model(model_path, batch=1):
    core = ov.Core()
    w = Path(model_path)
    ...
    print(f"Using OpenVINO {inference_mode} mode for batch={batch} inference...")
    ...
    return ov_compiled_model, inference_mode
...
def run_ov_model(model, inputs, inference_mode="LATENCY"):
    assert inference_mode == "LATENCY", "only support batch=1, if batch>1 you can reference Ultrlytics AutoBackend"
    y = list(model(inputs).values())
...
ov_compiled_model, inference_mode = load_openvino_model(model_path=model_path, batch=1)
...
for i, batch in enumerate(tqdm(dataloader, total=len(dataloader))):
    ...
    # 获取模型的输入层
    input_layer = next(iter(ov_compiled_model.inputs))
    ...
    if args.model_type == "yoloft":
        ...
        # 进行推理
        outputs = ov_compiled_model(inputs)
        pred, fmap2, fmap1, fmap0 = list(outputs.values())
        ...
        # 进行推理
        outputs = ov_compiled_model(inputs)
        pred = outputs[ov_compiled_model.output(0)]
